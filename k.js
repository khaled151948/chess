...
SquareObject.prototype.update = function () {
  this.element.className =
    ...
    (this.selected ? "selected" : "") +
    " " +
    (this.piece === null ? "empty" : this.piece.color + "-" + this.piece.type);
};
...
Castle.prototype.isValidMove = function (toSquare, n = 1) {
  ...
  if (movementX == 0 || movementY == 0) {
    let blocked = false;
    for (
      let testX = this.x + directionX, testY = this.y + directionY;
  ...
  if (n == 2 /*&&currentPlayer.checked==false*/) {
    for (let i = 0; i < pieces.length; i++) {
      if (pieces[i].color != currentPlayer.color) {
        ...
        if (
          pieces[i].isValidMove(
            getSquare(currentPlayer.king.x, currentPlayer.king.y),
            n - 1
          ).valid
        ) {
          result.valid = false;
...
Knight.prototype.isValidMove = function (toSquare, n = 1) {
  ...
  let result = { valid: false, capture: null };
  if (
    (Math.abs(movementX) == 2 && Math.abs(movementY) == 1) ||
      ...
      if (square.piece != null && pieces[i].captured == false) {
        if (pieces[i].color != currentPlayer.color) {
          if (pieces[i] instanceof Pawn) {
            ...
            if (this.color == "white") {
              if (
                Math.abs(black.king.x - toSquare.x) <= 1 &&
                Math.abs(black.king.y - toSquare.y) <= 1
              ) {
                result.valid = false;
                toSquare.setPiece(oldPiece);
                return result;
              ...
              if (
                Math.abs(white.king.x - toSquare.x) <= 1 &&
                Math.abs(white.king.y - toSquare.y) <= 1
              ) {
                result.valid = false;
                toSquare.setPiece(oldPiece);
                return result;
            ...
            if (
              pieces[i].isValidMove(getSquare(toSquare.x, toSquare.y)).valid
            ) {
              result.valid = false;
        ...
        if (currentPlayer.checked == false) {
          if (movementX == 2) {
            if (getSquare(8, Y).piece instanceof Castle) {
              if (
                getSquare(7, Y).piece == null &&
                getSquare(6, Y).piece == null
              ) {
            ...
            if (getSquare(1, Y).piece instanceof Castle) {
              if (
                getSquare(2, Y).piece == null &&
                getSquare(3, Y).piece == null &&
                getSquare(4, Y).piece == null
              ) {
                currentPlayer.kingMoved = true;
...
Pawn.prototype.isValidMove = function (toSquare, n = 1) {
  ...
  if (
    ...
    if (Math.abs(movementX) == 1) {
      if (toSquare.hasPiece() && toSquare.piece.color != this.color) {
        ...
        if (
          passantSquare.hasPiece() &&
          passantSquare.piece.color != this.color &&
          passantSquare.piece.type == "pawn" &&
          passantSquare.piece.advancedtwo == turn - 1
        ) {
  ...
  if (currentPlayer == white) {
    if (toSquare.y == 1 && this.y == 2) {
      if (
        (result.capture != null && Math.abs(movementX) == 1) ||
        (result.capture == null &&
          Math.abs(movementX) == 0 &&
          !toSquare.hasPiece())
      ) {
        console.log("called1");
        result.valid = true;
...
let move = function (start, end) {
  ...
  if (moveResult.valid) {
    ...
    if (currentPlayer == white) {
      if (
        end.piece.isValidMove(getSquare(black.king.x, black.king.y), 2).valid
      ) {
        showError("Check");
        black.checked = true;
      ...
      if (
        end.piece.isValidMove(getSquare(white.king.x, white.king.y), 2).valid
      ) {
        showError("Check");
        white.checked = true;
...
let isCheckmate = function (king) {
      ...
      if (king.x + i <= 8 && king.x + i >= 1) {
        if (king.y + j <= 8 && king.y + j >= 1) {
          ...
          if (i != 0 || j != 0) {
            ...
            if (
              getSquare(king.x + i, king.y + j).piece != null &&
              getSquare(king.x + i, king.y + j).piece.color ==
                currentPlayer.color
            ) {
              console.log("full");
              continue;
  ...
  for (let i = 0; i < pieces.length; i++) {
    if (currentPlayer.color == pieces[i].color) {
      if (
        pieces[i].isValidMove(getSquare(king.checkedBy.x, king.checkedBy.y), 2)
          .valid
      ) {
        console.log(pieces[i]);
      ...
      if (pieces[i] instanceof Pawn) {
        for (let dir = 1; dir <= 2; dir++) {
          ...
          if (pieces[i].isValidMove(square, 2).valid) {
            ...
            console.log(
              " to " + pieces[i].x,
              pieces[i].y + direction * dir + " valid "
            );
...
let promote = function (type) {
  ...
  if (index != -1) {
    ...
    if (currentPlayer == white) {
      if (
        newPiece.isValidMove(getSquare(black.king.x, black.king.y), 2).valid
      ) {
        showError("Check");
        black.checked = true;
      ...
      if (
        newPiece.isValidMove(getSquare(white.king.x, white.king.y), 2).valid
      ) {
        showError("Check");
        white.checked = true;
      }
...
let nextTurn = function () {
  ...
  if (currentPlayer.color == "white") {
    ...
    document.getElementById("turnInfo").innerHTML =
      "Player's turn: <b>Black</b>";
  } else {
    currentPlayer = white;
    document.getElementById("turnInfo").innerHTML =
      "Player's turn: <b>White</b>";
  }
};

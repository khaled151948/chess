window.onload = function () {
  ...
  function check() {
    if (myTurn) {
      ...
      for (var x = 0; x < scopes.length; x++) {
        sqs[scopes[x]].style.background = "#f45"; //.classList.add("scope"); // alert(scopes)
      }
    }
  }
  ...
  function chooseTurn() {
    ...
    for (var n = 0; n < 64; n++) {
      if ("prnbqk".indexOf(values[n]) >= 0) {
        ...
        for (var x = 0; x < scopes.length; x++) {
          ...
          var action = Math.random() * 3; //Action value
    ...
    var bestEffect = Math.min.apply(null, effects); //alert(bestEffect);
    ...
    bestMove = tmpB[tmpA.indexOf(Math.max.apply(null, tmpA))]; // alert(effects) //alert(bestMove);
